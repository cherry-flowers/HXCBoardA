...
/*========================= 文件依赖 ========================*/
#include "DMJ10010L.hpp"

/*================= 为DMJ10010L的静态成员分配内存 ===============*/

/** @brief 控制任务函数句柄,指向DMJ10010L的控制任务函数ControlTask */
TaskHandle_t DMJ10010L::ControlTaskHandle = nullptr;

/** @brief 静态实例注册表，存储所有DMJ10010L实例的指针 */
DMJ10010L* DMJ10010L::Instance_Registry[DMJ10010L_End-DMJ10010L_Begin] = {nullptr};
 ...
 */
DMJ10010L::DMJ10010L(DMJ10010L_ID motorid,
                     ...
                     DMJ10010L_ControlMode controlMode)
{
    ...
    /* 电机状态为失能 */
    this->Status = DMJ10010L_Status::DMJ10010L_DISABLE;
    /* 设置DMJ10010LID偏移量 */
    switch (ControlMode)
    {
    case DMJ10010L_MIT_Location_Mode:
    case DMJ10010L_MIT_Speed_Mode:
    case DMJ10010L_MIT_Torque_Mode:
        this->Motor_IDoffset = DMJ10010L_IDoffset::MIT_Offset;
        break;
    case DMJ10010L_Cascade_Loc_Mode:
        this->Motor_IDoffset = DMJ10010L_IDoffset::Cascade_LocPI_Offset;
        break;
    case DMJ10010L_Speed_Mode:
        this->Motor_IDoffset = DMJ10010L_IDoffset::Speed_PI_Offset;
        break;
    case DMJ10010L_Force_Loc_Mode:
        this->Motor_IDoffset = DMJ10010L_IDoffset::Cascade_LocPI_Limit_Offset;
        break;
    }
    /* 清空电机反馈 */
    ...
    /* 注册电机实例到注册表 */
    Instance_Registry[Motor_ID - DMJ10010L_Begin] = this; 
}   
...
DMJ10010L::~DMJ10010L(){
    /* 从实例注册表中移除 */
    Instance_Registry[Motor_ID - DMJ10010L_Begin] = nullptr;
    /* 取消绑定CanManager单例 */
 ...
 */
MW_Status DMJ10010L::Init(){
    /* 检查电机ID是否正确 */
    if(Motor_ID<DMJ10010L_Begin || Motor_ID>=DMJ10010L_End){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"电机ID错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 检查CAN总线是否正确 */
    if(CanBus<USE_CAN_BEGIN || CanBus>=USE_CAN_END){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"电机绑定的CAN总线不存在");
        return MW_Status::INVALID_PARAM;
    }
    ...
    /* 发送使能信号 */
    res = Send_Command(DMJ10010L_Command::Enable_Command);
    return res;
}

/**
 * @brief 启动DMJ10010L电机控制任务
 * @return MW_Status 启动结果
 * @note   该函数会创建一个任务,用于控制电机(多个电机也依旧是这个任务去管理)
 */
MW_Status DMJ10010L::Start_ControlTask(){
    /* 检查电机ID是否正确 */
    if(Motor_ID<DMJ10010L_Begin||Motor_ID>=DMJ10010L_End){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"电机ID错误");
        return MW_Status::INVALID_PARAM;
    ...
    {
        /* 启动控制任务 */
        xTaskCreateRes = xTaskCreate(ControlTask, "DMJ10010L Control Task", 128, NULL, 24, &ControlTaskHandle);
 ...
 * @note   用于发送DMJ10010L命令,将命令填充到ConmandMsg中
 */
MW_Status DMJ10010L::Send_Command(const DMJ10010L_Command& command){
    /* 检查CAN总线资源是否已初始化 */
    if(Resource_Inited == false){
        DMJ10010L_ASSERT(MW_Status::ERROR,"CAN总线资源未初始化");
    ...
    switch (command)
    {
    case DMJ10010L_Command::Enable_Command:
        /* 构建使能命令 */
        Build_EnableCommand(Msg);
        break;
    case DMJ10010L_Command::Disable_Command:
        /* 构建失能命令 */
        Build_DisableCommand(Msg);
        break;
    case DMJ10010L_Command::SaveZero_Command:
        /* 构建保存零点命令 */
        Build_SaveZeroPosCommand(Msg);
        break;
    case DMJ10010L_Command::ClearErr_Command:
        /* 构建清除错误命令 */
 ...
 * @warning Kd的范围为(0,5]
 */
MW_Status DMJ10010L::Set_MIT_Location_Mode_Expect(float32_t Exp_Rad,float32_t Exp_Speed,float32_t Exp_Torque,float32_t Kp,float32_t Kd)
{
    /* 检查是否是MIT_Location_Mode模式 */
    if(ControlMode != DMJ10010L_MIT_Location_Mode){
        DMJ10010L_ASSERT(MW_Status::INVALID_OPERATION,"不是MIT_Location_Mode模式,操作失败");
        return MW_Status::INVALID_OPERATION;
    }
    /* 检查Exp_Rad参数是否合法 */
    if(Exp_Rad<-PMAX || Exp_Rad>PMAX){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Exp_Rad参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 检查Exp_Speed参数是否合法 */
    if(Exp_Speed<-VMAX || Exp_Speed>VMAX){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Exp_Speed参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 检查Exp_Torque参数是否合法 */
    if(Exp_Torque<-TMAX || Exp_Torque>TMAX){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Exp_Torque参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 检查Kp参数是否合法 */
    if(Kp<=0 || Kp>500){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Kp参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 检查Kd参数是否合法 */
    if(Kd<=0 || Kd>5){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Kd参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 更新DMJ10010L_Data中的用户设置期望值 */
    ...
    MotorData.MIT_Kp_Map = (uint16_t)FloatToUint(MotorData.Kp,DMJ10010L_CharacterParam::KpMin,DMJ10010L_CharacterParam::KPMax,12);
    /* Kd参数(12位数据) = Kd / 5.0f*4095.0f */
    MotorData.MIT_Kd_Map = (uint16_t)FloatToUint(MotorData.Kd,DMJ10010L_CharacterParam::KdMin,DMJ10010L_CharacterParam::KdMax,12);
 ...
 * @warning Kd的范围为(0,5]
 */
MW_Status DMJ10010L::Set_MIT_Speed_Mode_Expect(float32_t Exp_Speed,float32_t Exp_Torque, float32_t Kd)
{
    /* 检查是否是MIT_Speed_Mode模式 */
    if(ControlMode != DMJ10010L_MIT_Speed_Mode){
        DMJ10010L_ASSERT(MW_Status::INVALID_OPERATION,"不是MIT_Speed_Mode模式,操作失败");
        return MW_Status::INVALID_OPERATION;
    }
    /* 检查Exp_Speed参数是否合法 */
    if(Exp_Speed<-VMAX || Exp_Speed>VMAX){
 ...
 */
MW_Status DMJ10010L::Set_MIT_Torque_Mode_Expect(float32_t Exp_Torque)
{
    /* 检查是否是MIT_Torque_Mode模式 */
    if(ControlMode != DMJ10010L_MIT_Torque_Mode){
        DMJ10010L_ASSERT(MW_Status::INVALID_OPERATION,"不是MIT_Torque_Mode模式,操作失败");
        return MW_Status::INVALID_OPERATION;
    }
    /* 检查Exp_Torque参数是否合法 */
    if(Exp_Torque<-TMAX || Exp_Torque>TMAX){
 ...
 */
MW_Status DMJ10010L::Set_Cascade_Loc_Mod_Expect(float32_t Exp_Rad,float32_t SpeedMax)
{
    /* 检查是否是Cascade_Loc_Mode模式 */
    if(ControlMode != DMJ10010L_Cascade_Loc_Mode){
        DMJ10010L_ASSERT(MW_Status::INVALID_OPERATION,"不是Cascade_Loc_Mode模式,操作失败");
        return MW_Status::INVALID_OPERATION;
    }
    /* 检查SpeedMax参数是否合法 */
    if(SpeedMax<=0.0f || SpeedMax>DMJ10010L_CharacterParam::IDLE_GearBoxRad){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"SpeedMax参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 更新DMJ10010L_Data中的用户设置期望值 */
    MotorData.Exp_Rad = Exp_Rad;
 ...
 */
MW_Status DMJ10010L::Set_Speed_Mode_Expect(float32_t Exp_Speed){
    /* 检查是否是Speed_Mode模式 */
    if(ControlMode != DMJ10010L_Speed_Mode){
        DMJ10010L_ASSERT(MW_Status::INVALID_OPERATION,"不是Speed_Mode模式,操作失败");
        return MW_Status::INVALID_OPERATION;
    }
    /* 检查Exp_Speed参数是否合法 */
    if(Exp_Speed<-DMJ10010L_CharacterParam::IDLE_GearBoxRad || Exp_Speed>DMJ10010L_CharacterParam::IDLE_GearBoxRad){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Exp_Speed参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 更新DMJ10010L_Data中的用户设置期望值 */
 ...
 */
MW_Status DMJ10010L::Set_Force_Loc_Mode_Expect(float32_t Exp_Rad,float32_t Speed_Limit,float32_t Torque_MAX_Rate)
{
    /* 检查是否是Force_Loc_Mode模式 */
    if(ControlMode != DMJ10010L_Force_Loc_Mode){
        DMJ10010L_ASSERT(MW_Status::INVALID_OPERATION,"不是Force_Loc_Mode模式,操作失败");
        return MW_Status::INVALID_OPERATION;
    }
    /* 检查Speed_Limit参数是否合法 */
    if(Speed_Limit<=0.0f || Speed_Limit>100.0f){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Speed_Limit参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 检查Torque_MAX_Rate参数是否合法 */
    if(Torque_MAX_Rate<0.0f || Torque_MAX_Rate>1.0f){
        DMJ10010L_ASSERT(MW_Status::INVALID_PARAM,"Torque_MAX_Rate参数错误");
        return MW_Status::INVALID_PARAM;
    }
    /* 更新DMJ10010L_Data中的用户设置期望值 */
    MotorData.Exp_Rad = Exp_Rad;
...
/**
 * @brief 获取DMJ10010L反馈消息
 * @return DMJ10010L_FeedBackMsg DMJ10010L反馈消息结构体
 * @note  该函数用于获取电机反馈信息
 */
DMJ10010L_FeedBackMsg DMJ10010L::getFeedBackMsg() const{
    DMJ10010L_FeedBackMsg temp;
    /* 记录当前中断状态 */
    uint32_t primask_bit = __get_PRIMASK();
    __disable_irq();
    /* 安全地拷贝 volatile 数据到临时变量 */
    temp = const_cast<const DMJ10010L_FeedBackMsg&>(this->FeedBackMsg);
    /* 退出临界区（恢复中断）*/
    if (!primask_bit) {
        __enable_irq();
    }
...
/**
 * @brief 获取DMJ10010L电机的数据参数
 * @return DMJ10010L_Data 电机数据参数结构体
 * @note  该函数用于获取电机的实时数据参数
 */
DMJ10010L_Data DMJ10010L::getMotorData() const{
    DMJ10010L_Data temp;
    /* 记录当前中断状态 */
    uint32_t primask_bit = __get_PRIMASK();
    __disable_irq();
    /* 安全地拷贝 volatile 数据到临时变量(一次拷贝) */
    temp = const_cast<const DMJ10010L_Data&>(this->MotorData);
    /* 退出临界区（恢复中断）*/
    if (!primask_bit) {
        __enable_irq();
    }
...
/**
 * @brief  DMJ10010L电机CAN初始化函数,申请CAN总线资源
 * @return MW_Status 初始化状态
 * @note   被Init调用,申请CAN总线资源，Init已经保证电机ID和CAN总线的有效性
 */
MW_Status DMJ10010L::InitResource()
{
    /* 申请CAN总线资源 */
    MW_Status res = CanManagerPtr->AskResource(CanBus,BaudRate,Mode);

    /* 检查CAN总线资源申请结果 */
    DMJ10010L_ASSERT(res,"申请CAN总线资源失败,检查CAN总线配置和之前配置是否冲突");
    /* 如果申请成功,则启动CAN总线资源 */
    if(res == MW_Status::SUCCESS)
    {
        res = CanManagerPtr->StartResource(CanBus);
        /* 检查CAN总线资源启动结果 */
        DMJ10010L_ASSERT(res,"启动CAN总线资源失败");
        /* 如果CAN总线资源启动成功 */
        if(res == MW_Status::SUCCESS){
            /* 硬件资源初始化成功 */
 ...
 */
MW_Status DMJ10010L::SetCallBack(){
    MW_Status res = MW_Status::ERROR;
    /* 检查CAN总线资源是否已初始化 */
    if(Resource_Inited == true){
        /* 订阅CAN总线,向CanManager 注册CAN消息的回调函数,用于处理DMJ10010L电机返回的CAN消息 */
        res = CanManagerPtr->Subscribe(CanBus,Feedback_ID,DMJ10010L_CanMsgCallBack);
        /* 检查CAN消息回调函数订阅结果 */
        DMJ10010L_ASSERT(res,"订阅CAN消息回调函数失败");
    }
    return res;
}
 ...
 */
void DMJ10010L::Build_EnableCommand(CanMessage& ConmandMsg){
    ConmandMsg.id = Motor_ID + Motor_IDoffset;
 ...
 */
void DMJ10010L::Build_DisableCommand(CanMessage& ConmandMsg){
    ConmandMsg.id = Motor_ID + Motor_IDoffset;
    ConmandMsg.isExtended = false;
    ConmandMsg.isRemote = false;
    ConmandMsg.len = 8;
 ...
 */
void DMJ10010L::Build_SaveZeroPosCommand(CanMessage& ConmandMsg){
    ConmandMsg.id = Motor_ID + Motor_IDoffset;
    ConmandMsg.isExtended = false;
    ConmandMsg.isRemote = false;
    ConmandMsg.len = 8;
 ...
 */
void DMJ10010L::Build_ClearErrCommand(CanMessage& ConmandMsg){
    ConmandMsg.id = Motor_ID + Motor_IDoffset;
    ConmandMsg.isExtended = false;
    ConmandMsg.isRemote = false;
    ConmandMsg.len = 8;
 ...
 * @return MW_Status 发送状态
 */
MW_Status DMJ10010L::Send_ControlCommand(){
    /* 检查CAN总线资源是否已初始化 */
    if(Resource_Inited == false){
        return MW_Status::ERROR;
    }
    ...
    CanMessage Msg;
    Msg.id = Motor_ID + Motor_IDoffset;
...
/**
 * @brief DMJ10010L电机的CAN消息回调函数
 * @param canId 收到的CAN消息ID
 * @param data 收到的CAN消息数据指针
 * @param len 收到的CAN消息数据长度
 * @details 根据反馈的C620消息，刷新电机数据
 * @note  工作流程：校验数据格式 -> 赋值给fanDMJ10010L反馈消息结构体 -> 解析数据 -> 刷新电机数据
 */
void DMJ10010L::DMJ10010L_CanMsgCallBack(uint32_t canId, uint8_t* data, uint8_t len){
    /* 检查CAN消息数据是否有效 */
    if(data == nullptr || len != 8){
        return ;
    }
    /* 检查ID是否在范围内 */
    if(canId < DMJ10010L_MST_Begin || canId >= DMJ10010L_MST_End){
        return;
    }
    /* 寻找当前电机实例,根据CAN消息ID确定电机实例 */
    DMJ10010L* temp = Instance_Registry[canId - DMJ10010L_MST_Begin];
    if(temp != nullptr){
        ...
        /* 刷新电机状态 */
        temp->Status = (DMJ10010L_Status)(ERR);
        /* 刷新电机上一次POS值 */
        ...
        if(Delta_Encoder < -DMJ10010L_CharacterParam::Encoder_Per_Round/2){
            /* POS正方向转了一圈 */
            temp->MotorData.Total_EncoderRound++;
        }/* POS增量 > Encoder_Per_Round/2, 则POS负方向转了一圈 */
        else if(Delta_Encoder > DMJ10010L_CharacterParam::Encoder_Per_Round/2){
            /* POS负方向转了一圈 */
            temp->MotorData.Total_EncoderRound--;
        }
        /* 总的编码器计数 = 总圈数 * 每圈编码器数 + 当前编码器值 */
        temp->MotorData.Total_Encoder = temp->MotorData.Total_EncoderRound * DMJ10010L_CharacterParam::Encoder_Per_Round + temp->MotorData.Now_Encoder;
        /* 电机输出轴当前弧度  = 总的编码器圈数*2*PMAX + 当前编码器值/(每圈编码器最大值-1)*2*PMAX-PMAX */
        temp->MotorData.Now_Rad = temp->MotorData.Total_EncoderRound * 2 * temp->PMAX + temp->MotorData.Now_Encoder / (float32_t)(DMJ10010L_CharacterParam::Encoder_Per_Round-1) * 2 * temp->PMAX - temp->PMAX;
        ...
        temp->MotorData.Now_Speed =  VEL * 2 * temp->VMAX / (float32_t)(DMJ10010L_CharacterParam::Speed_Per_Round-1) - temp->VMAX;
        /* 电机输出轴扭矩 = T * 2 * TMAX/(每圈扭矩最大值-1) - TMAX*/
        temp->MotorData.Now_Torque = T* 2 * temp->TMAX / (float32_t)(DMJ10010L_CharacterParam::Torque_Per_Round-1) - temp->TMAX;
        /* 电机MOS平均温度 */
...
/**
 * @brief 控制任务函数,用于根据控制模式,控制所有注册的DMJ10010L电机
 */
void DMJ10010L::ControlTask(void* pvParameters){
    /* 任务运行频率为1ms */
    const TickType_t xFrequency = pdMS_TO_TICKS(1);
    /* 任务第一次运行时,获取当前时间 */
    TickType_t xLastWakeTime = xTaskGetTickCount();
    /* 定义DMJ10010L电机实例指针(临时变量)*/
    DMJ10010L* Instance = nullptr;
    while(1)
    {
        /* 遍历所有的DMJ10010L电机实例 */
        for(uint8_t index = DMJ10010L_Begin;index<DMJ10010L_End;index++){
            Instance = Instance_Registry[index - DMJ10010L_Begin];
            if(Instance != nullptr){
                /* 调用电机实例的控制函数 */
